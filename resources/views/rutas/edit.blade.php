      ...
      <div class="row mb-4">
        <div class="col-md-6">
          ...
          <div class="input-group mb-2">
            <input type="text" id="latO_vis" class="form-control" readonly
                   value="{{ old('latitud_origen',$ruta->latitud_origen) }}">
            <input type="text" id="lngO_vis" class="form-control" readonly
        ...
        <div class="col-md-6">
          ...
          <div class="input-group mb-2">
            <input type="text" id="latD_vis" class="form-control" readonly
                   value="{{ old('latitud_destino',$ruta->latitud_destino) }}">
            <input type="text" id="lngD_vis" class="form-control" readonly
        ...
        <div class="dual-list">
          <ul id="available-stops">
            @foreach($allStops as $stop)
              @if(! $ruta->stops->pluck('id')->contains($stop->id))
...
@push('scripts')
  ...
  <script>
    // Leaflet
    const o=[{{ $ruta->latitud_origen }},{{ $ruta->longitud_origen }}],
          d=[{{ $ruta->latitud_destino }},{{ $ruta->longitud_destino }}];
    const map=L.map('map').setView(o,6);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:18}).addTo(map);
    let mO=L.marker(o).addTo(map), mD=L.marker(d).addTo(map),
        line=L.polyline([o,d],{color:'#666'}).addTo(map),
        step=0;
    ...
    map.on('click', e=>{
      ...
      if(step===0){
        mO.setLatLng(e.latlng); latO.value=lat; lngO.value=lng; step=1;
      } else if(step===1){
        mD.setLatLng(e.latlng); latD.value=lat; lngD.value=lng; line.setLatLngs([mO.getLatLng(),mD.getLatLng()]); 
      }
      updateCoords();
    });
    ...
    Sortable.create(avail,{group:{name:'stops',pull:true,put:false},animation:150});
    Sortable.create(sel,{group:{name:'stops',pull:true,put:true},animation:150,onAdd:sync,onUpdate:sync,onRemove:sync});
    function sync(){
      inputs.innerHTML='';
